...
impl TestSession {
    ...
    pub fn test_set_get_memo(&mut self, txid: &str, old: &str, new: &str) {
        ...
        assert!(self.session.set_transaction_memo(0, txid, new, 1).is_err());
        assert!(self.session.set_transaction_memo(0, txid, &"a".repeat(1025), 0).is_err());
        assert!(self.session.set_transaction_memo(0, txid, new, 0).is_ok());
    ...
    pub fn check_decryption(&mut self, tip: u32, txids: &[&str]) {
        ...
        for txid in txids {
            assert!(account0.all_txs.get(&Txid::from_hex(txid).unwrap()).is_some())
        }
    }
